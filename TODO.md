...
- **[PROPOSE] `feedback_philosophy.md` §17 should record the `St`/`Fu` relationship.** Not edited — §17 is settled and this is a real change to the unit model. The claim to add: work delivered is force over hardness, so a machine states force and the material decides what that accomplishes.
